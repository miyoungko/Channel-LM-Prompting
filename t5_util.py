...
def load_checkpoint(t5, checkpoint=None,
    ...
    if checkpoint is not None:
        ...
        if prompt_tune:
            ...
            weight = torch.load(checkpoint)["shared.new_embed.weight"]
...
def set_extra_embeddings(model, n_prefix):
    model.set_input_embeddings(
        MyEmbedding(model.shared, n_prefix))
...
def set_separate_embeddings(model, mapping):
    model.set_input_embeddings(
        MyEmbedding2(model.shared, mapping))
